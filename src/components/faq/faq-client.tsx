            ...
            <Accordion type="single" collapsible className="w-full space-y-4">
              {category.items.map(item => (
                <AccordionItem value={item.question} key={item.question} className="rounded-2xl border bg-card p-1 shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5">
                  <AccordionTrigger className="px-6 py-4 text-left font-semibold text-lg hover:no-underline">{t(item.question as any)}</AccordionTrigger>
                  <AccordionContent className="px-6 text-muted-foreground text-base">{t(item.answer as any)}</AccordionContent>
                </AccordionItem>
              ))}
            </Accordion>
          </div>
